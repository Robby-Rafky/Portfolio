document.addEventListener("DOMContentLoaded", () => {
  ...
  const projectDescriptions = {
    "Cover Letter Agent" : "AI Cover Letter Generator: Developed a full-stack app using Python, including a desktop client with PySide6 and a backend powered by FastAPI. The backend handles user authentication, applicant data storage (PostgreSQL + SQLAlchemy), and CV parsing via an AI agent that extracts relevant details for cover letter generation. Integrated a zero-shot LangChain agent using tool use and reasoning-before-acting through Together AI's LLaMA 3 API. Added async threading, input validation, and UX features like save/load and clipboard export. Future plans include transitioning to a React frontend to enhance UI experience.",
  ...
  const projectLibraryData = {
    "Cover Letter Agent": ["Python", "Langchain", "FastAPI", "PostgreSQL", "SQLAlchemy", "pyside6", "Together.ai"],
